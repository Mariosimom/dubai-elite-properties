...
export type Property = {
  ...
  image: string;
  images?: string[];
  // Dubai standards (optional)
  reraNo?: string;
  permitNo?: string;
  dedLicense?: string;
  brokerReraCard?: string;
...
export default function PropertyCard({ p }: { p: Property }) {
  ...
  // ✅ Safe fallbacks so we never pass an empty src to <Image/>
  const imageSrc = p.image && p.image.trim() ? p.image : PLACEHOLDER;
  const gallery =
    (p.images && p.images.length ? p.images : [imageSrc]).map((u) =>
      u && u.trim() ? u : PLACEHOLDER
    );
      ...
      <div className="card overflow-hidden cursor-pointer" onClick={() => setOpen(true)}>
        ...
        <div className="p-4 space-y-2">
          ...
          <div className="text-sm text-gray-600">
            {p.beds} Beds • {p.baths} Baths • {p.area} m²
          </div>
        </div>
      </div>
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="text-sm text-gray-500">{p.location}</div>
              {p.developer && (
                <div className="text-sm text-gray-600">Developer: {p.developer}</div>
              )}
              {p.handover && (
                <div className="text-sm text-gray-600">Handover: {p.handover}</div>
              )}
            </div>
            <div className="text-brand-600 font-semibold">{p.priceAED}</div>
          </div>
          ...
          <div className="mt-2 flex justify-end">
            <a href="#contact" className="rounded-xl bg-brand-600 text-white px-4 py-2">
              Enquire
            </a>
          </div>
        </div>
